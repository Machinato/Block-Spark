...
export const Beam = forwardRef(({ children, position, stride = 4, width = 8, ...props }, fRef) => {
  ...
  useFrame(() => {
    ...
    // console.log('range:', range, 'positions:', Array.from(reflect.current.positions.slice(0, range * 3 + 3)))
    ...
    if (streaks.current.instanceMatrix.updateRange) {
    ...
    for (i = 1; i < range; i++) {
      ...
      obj.scale.setScalar(i === 1 ? 3.0 : 0.75)
    ...
    if (glow.current.instanceMatrix.updateRange) {
